...
212 ╘   211 ╙
*/
let edges = ["corner", "edge", "middle"];

function drawAnyPizza(length, width, edgeType:string):string {
    let topLefts = ['╔', '╓', '┌'];      // corner, edge, middle
    let tops = ['═', '─', '─'];      // corner, edge, middle
    let lefts = ['║', '║', '│'];
    let bottomLefts = ['╙', '╙', '└'];

    let pizza = topLefts[edges.indexOf(edgeType)];
    let bottom = bottomLefts[edges.indexOf(edgeType)];
    ...
    for (let j = 0; j < length-1; j++) {    // Rando generate middle portions
        let middle = lefts[edges.indexOf(edgeType)];
        for (let i = 0; i < width; i++) {
            let toppingNum = Math.round( ( Math.random()*20));//**( Math.floor(Math.random()*23) ) ) *111);
            let topping='';
            switch (toppingNum) {
                case 0:topping=' ';break;
                case 1:topping='∙';break;
                case 2:topping='°';break;
                case 3:topping='Θ';break;
                case 4:topping='@';break;   //onion
                case 5:topping='*';break;
                case 6:topping='Φ';break;   //tomato
 ...
 */
​
function drawMediumPizzaSlice(sliceLocation:string) {
//    return  ` __ \n`+
//            `|__|`;
...
​
 */
function drawLargePizzaSlice(sliceLocation:string) {
//    return  ` __ \n`+
//            `| .|\n`+
//            `|__|`;
