	function numberToWords (amount:number):string {
		let digitWords:Array<string> = 	["", 	"one",		"two",		"three",	"four",		"five",		"six",		"seven",	"eight",	"nine"];
		let teensWords:Array<string> = 	["ten",	"eleven",	"twelve",	"thirteen",	"fourteen",	"fifteen",	"sixteen",	"seventeen","eighteen",	"nineteen"];
		let tensWords:Array<string> = 	["", 	"",			"twenty",	"thirty",	"forty",	"fifty",	"sixty",	"seventy",	"eighty",	"ninety"];
		let suffixes:Array<string> = 	["",	"thousand ",	"million ",	"billion ",	"trillion ",	"quadrillion "];
		
		let numberWord:string = "";
		
		let amountAsString:string = String(amount);//String.format("%.2f", amount); // Turn the double into a String with two decimal places 
//		System.out.println(amountAsString);

		let dollarAmountAsString = "";
		let centsAmountAsString = "00";

		let decimalIndex:number = amountAsString.indexOf('.');
		if (decimalIndex == -1) decimalIndex = amountAsString.length;						// No decimal found in number, so set it to the end
		...
		let numSections:number = dollarAmountAsString.length / 3 + 1;	// Get count of thousands divisions ($x,xxx,xxx,xxx etc) (integer division truncates fractional part)
		let numLeftoverDigits:number = dollarAmountAsString.length % 3;	// Get the # of front-end digits if not a triplet
		if (numLeftoverDigits === 0) {numSections--; numLeftoverDigits = 3;}

		let leftoverNumsStr:string = dollarAmountAsString.substring(0, numLeftoverDigits);	// Get those front-end digits as a String (empty if none)
		let digitSection:number = parseInt(leftoverNumsStr);	// Turn that string back into a number
		...
		case 3:
			let firstDigit = digitSection / 100;
			numberWord = numberWord.concat( digitWords[firstDigit]+" ");
			if (firstDigit != 0) numberWord = numberWord.concat("hundred ");
		case 2:
			...
			if (digitSection > 9 && digitSection < 20) {
				...
				if (digitSection > 19) numberWord = numberWord.concat(tensWords[ (digitSection / 10) ]);	// Again, integer division, so result will be tens place digit
				if (digitSection % 10 == 0) numberWord = numberWord.concat(digitSection / 10 == 0 ? "" : " ");
				else numberWord = numberWord.concat(digitSection < 10 ? "" : "-");
			}	// Hopefully the else statement will drop through to "case 1:"
		case 1:
			digitSection %= 10;			// Drop the 10s part
			if (digitSection != 0) numberWord = numberWord.concat( digitWords[digitSection]+" ");		// concat "one" through "nine" ("zero" is blank as it should be)
			break;
		case 0:break;
		default:break;
		}
		...
		if (numLeftoverDigits == 3) numLeftoverDigits = 0;
		for (let i = 0; i < numSections-1; i ++) {
			let currentSectionStr:string = (i == numSections-1) ?						// Grab next section from amount string
			...
			// Start with the front-end digits, because they may not be a triplet. We don't break the case statements as we want them to fall through to next
			let numDigits = String(digitSection).length;
			switch (numDigits) {
			case 3:
				let firstDigit = digitSection / 100;
				numberWord = numberWord.concat( digitWords[firstDigit]+" ");
